...
-- individually centered (since txtWrap just left-aligns inside its box)
...
-- stolen from about.md's closing line, todo, actually add a site meta desc.
siteDesc :: Text
siteDesc = " · yapping about programming, security, & whatever else"

-- brand + site desc + greeting on one line; the desc drops to its own
-- wrapped line(s) when the terminal is too narrow
...
-- right-aligned key hints, flowing onto extra lines on narrow terminals
