...
WORKDIR /build
COPY kio-tui.cabal ./
# Best-effort dep layer so content/code edits don't recompile brick & co.
...
COPY logo.txt ./
COPY app app
# cabal install builds from an sdist copy, which breaks relative content
# embed paths. build in place instead.
RUN cabal build exe:kio-tui && cp "$(cabal list-bin kio-tui)" /out/kio-tui && strip /out/kio-tui
# Smoke-test listing all content by running non-tty.
RUN /out/kio-tui </dev/null >/dev/null

FROM debian:bookworm-slim
RUN apt-get update \
  && apt-get install -y --no-install-recommends \
  # ncurses-term: terminfo for modern terminals that ncurses-base lacks;
  ...
  && rm -rf /var/lib/apt/lists/* \
  # ForceCommand runs through user's shell, so it must be a real one
